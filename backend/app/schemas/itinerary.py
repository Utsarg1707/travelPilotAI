...
class Attraction(BaseModel):
    ...
    category: str = Field(
        "General", description="Category (Beach, Cultural, Food, Shopping, Nature)"
    )
    description: str = Field("", description="Brief description")
    estimated_duration: str = Field("2 hours", description="Estimated visit duration")
    estimated_cost_inr: float = Field(
        0.0, ge=0.0, description="Ticket/entry cost per person in INR"
    )
...
class ItineraryDay(BaseModel):
    ...
    activities: list[ItineraryActivity] = Field(
        default_factory=list, description="Scheduled activities"
    )
...
class Itinerary(BaseModel):
    ...
    destinations: list[str] = Field(
        default_factory=list, description="List of all destinations included in trip"
    )
    total_days: int = Field(..., ge=1, description="Total trip duration in days")
    days: list[ItineraryDay] = Field(
        default_factory=list, description="Day-by-day itinerary breakdown"
    )
